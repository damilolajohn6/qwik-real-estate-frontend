...
export default function AgentListingsActive() {
  ...
  const [filters, setFilters] = useState({
    ...
    squareFootageMin: "",
    squareFootageMax: "",
    status: "active", // Default to active, options will be active or pending
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch properties with filters, restricting to active/pending statuses
        const queryParams = {
          page: page.toString(),
        ...
        try {
          ...
          if (propertiesData && Array.isArray(propertiesData.properties)) {
            // Filter for active/pending listings
            const activePendingProperties = propertiesData.properties.filter(
              (prop: Property) =>
                prop.status === "active" || prop.status === "pending"
            );
            setProperties(activePendingProperties);
            setTotalPages(
              Math.ceil(activePendingProperties.length / limit) || 1
            );
          } else {
            setProperties([]);
            setTotalPages(1);
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F5F2]">
      ...
      <div className="flex-1 p-4">
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>Filter Agent Listings</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle>Agent Listings</CardTitle>
          ...
          <CardContent>
            {properties.length === 0 ? (
              <p className="text-center text-gray-500">
                No active/pending listings found.
              </p>
            ) : !isMobile ? (
              <Table>
                <TableHeader>
